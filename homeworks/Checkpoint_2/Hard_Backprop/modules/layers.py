...
class Linear(Module):
    ...
    def __repr__(self) -> str:
        ...
        return f'Linear(in_features={in_features}, out_features={out_features}, bias={not self.bias is None})'
...
class BatchNormalization(Module):
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        if self.training:
            ...
            dx_norm = grad_output
            dvar = (dx_norm * self.input_mean * (-0.5) * self.inv_sqrt_var ** 3).sum(axis=0)
            dmean = (-dx_norm * self.inv_sqrt_var).sum(axis=0) + dvar * (-2 * self.input_mean).mean(axis=0)
            return dx_norm * self.inv_sqrt_var + dvar * 2 * self.input_mean / B + dmean / B
        else:
            if self.affine:
                grad_output = grad_output * self.weight
...
class Dropout(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        if self.training:
            self.mask = (np.random.uniform(size=input.shape) > self.p).astype(input.dtype)
